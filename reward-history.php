...
$buildFilterUrl = static function (array $overrides = []) use ($status, $source, $phase, $flow) {
    ...
    return BASE_URL . '/reward-history.php?' . http_build_query($query);
};
    ...
    <div class="reward-page-shell reward-history-shell">
        <section class="reward-panel">
            <div>
                ...
                <h1>Track every reward movement with confidence</h1>
                <p>Review incoming rewards, locked balances, and outgoing claim movements from a cleaner premium history workspace built for clarity.</p>
                <div class="page-actions">
                    <a href="<?php echo BASE_URL; ?>/dashboard.php" class="secondary-btn">Back to Dashboard</a>
                    <a href="<?php echo BASE_URL; ?>/notifications.php?status=all" class="secondary-btn">Notifications</a>
                </div>
            </div>
            <div class="reward-balance-box">
                <span>Wallet-facing balance</span>
        ...
        <section class="reward-history-stats">
            ...
            <div class="reward-history-stat"><span>Claimed / Outgoing</span><strong><?php echo number_format((float) $balances['claimed'], 2); ?> $REX</strong></div>
        </section>
        ...
        <section class="reward-history-grid">
            <div class="reward-history-table-card">
                <div class="reward-history-filter-head">
                    <div>
                        ...
                        <p>Use focused filters to quickly isolate the reward movements you want to review.</p>
                    </div>
                    <div class="status-chip"><?php echo (int) $summary['total_rows']; ?> total ledger entr<?php echo (int) $summary['total_rows'] === 1 ? 'y' : 'ies'; ?></div>
                </div>
                ...
                <form method="GET" class="reward-history-toolbar">
                    ...
                    <div class="page-actions" style="grid-column:1/-1;">
                        ...
                        <a href="<?php echo BASE_URL; ?>/reward-history.php" class="secondary-btn">Reset</a>
                    </div>
                </form>
            ...
            <div class="reward-side-stack">
                ...
                <section class="reward-history-claim-card">
                    ...
                    <p><?php echo htmlspecialchars((string) ($claim_eligibility['message'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></p>
                    <div class="reward-readiness-grid">
                        <div class="claim-metric"><span>Incoming total</span><strong><?php echo number_format((float) ($summary['incoming_total'] ?? 0), 2); ?></strong></div>
                        <div class="claim-metric"><span>Outgoing total</span><strong><?php echo number_format((float) ($summary['outgoing_total'] ?? 0), 2); ?></strong></div>
                        <div class="claim-metric"><span>Claim threshold</span><strong><?php echo number_format((float) REWARD_CLAIM_MINIMUM_REX, 2); ?></strong></div>
                        <div class="claim-metric"><span>Abuse review</span><strong><?php echo !empty($claim_eligibility['eligible']) ? 'Passed' : 'Monitoring'; ?></strong></div>
                    </div>
                ...
                <section class="reward-history-claim-card">
                    ...
                    <p>Prepared and historical claim batches are listed here for visibility into outgoing reward movements.</p>
                    <div class="reward-claim-history">
                        <?php if (empty($claim_rows)): ?>
                            <div class="reward-history-empty">No claim snapshots have been created yet.</div>
                        <?php else: ?>
