...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $current_user) {
    ...
    if ($action === 'toggle_like') {
        ...
        redirect(BASE_URL . '/public/reviews.php?page=' . max(1, (int) ($_GET['page'] ?? 1)));
    }
                ...
                <?php else: ?>
                    <section class="reviews-feed two-col">
                        <?php foreach ($reviews as $index => $review): ?>
                            ...
                            ?>
                            <article class="review-card <?php echo $is_top ? 'top-review' : ''; ?>" data-full-content="<?php echo esc($full_content_raw); ?>">
                                ...
                                <div class="project-row">
                                    <a href="<?php echo BASE_URL; ?>/public/project-detail.php?id=<?php echo (int)$review['project_id']; ?>" class="project-info no-logo">
                                ...
                                <div class="review-content-wrap">
                                    ...
                                    <?php
                                    $full_content = trim((string)($review['review_content'] ?? ''));
                                    $needs_modal = !$is_public_view && strlen($full_content) > 180;
                                    ?>
                                    <?php if ($needs_modal): ?>
                                        <button type="button" class="read-more-btn" data-review-id="<?php echo (int) $review['id']; ?>">Read Full Review</button>
                                    <?php endif; ?>
                                </div>
                    ...
                    <?php if ($total_pages > 1): ?>
                        <nav class="reviews-pagination" aria-label="Reviews pagination">
                            ...
                            <?php $qs['page'] = $prev; ?>
                            <a class="page-btn <?php echo $page <= 1 ? 'disabled' : ''; ?>" href="<?php echo $page <= 1 ? '#' : (BASE_URL . '/public/reviews.php?' . http_build_query($qs)); ?>"><i class="fas fa-chevron-left"></i> Prev</a>
                            <span class="page-info">Page <?php echo $page; ?> of <?php echo $total_pages; ?></span>
                            <?php $qs['page'] = $next; ?>
                            <a class="page-btn <?php echo $page >= $total_pages ? 'disabled' : ''; ?>" href="<?php echo $page >= $total_pages ? '#' : (BASE_URL . '/public/reviews.php?' . http_build_query($qs)); ?>">Next <i class="fas fa-chevron-right"></i></a>
                        </nav>
                    <?php endif; ?>
                <?php endif; ?>
            </section>
...
<div id="reviewModal" class="review-modal" role="dialog" aria-modal="true" aria-hidden="true">
    ...
    <div class="review-modal__panel">
        ...
        <div class="review-modal__body">
            <!-- Header Row: Avatar + Name+Level | Rating -->
            <div class="review-modal__header">
                <div class="review-modal__user">
                    <div class="review-modal__avatar-wrap">
                        <img src="" alt="" class="review-modal__avatar" id="modalAvatar">
                        <div class="review-modal__avatar-placeholder" id="modalAvatarPlaceholder" hidden></div>
                    </div>
                    <div class="review-modal__user-meta">
                        <div class="review-modal__name-row">
                            <strong class="review-modal__username" id="modalUsername"></strong>
                            <span class="review-modal__level-badge" id="modalLevelBadge"></span>
                        </div>
                        <span class="review-modal__date" id="modalDate"></span>
                    </div>
                </div>
                <div class="review-modal__rating" id="modalRating"></div>
            </div>

            <!-- Project Row: Label + Name (left) | Statuses (right) -->
            <div class="review-modal__project">
                <span class="project-label">Project</span>
                <div class="review-modal__project-row">
                    <a href="" class="review-modal__project-link" id="modalProjectLink">
                        <span class="review-modal__project-name" id="modalProjectName"></span>
                    </a>
                    <div class="review-modal__project-statuses" id="modalProjectStatuses"></div>
                </div>
            </div>

            <!-- Title -->
            <h3 class="review-modal__title" id="modalTitle"></h3>

            <!-- Full Content -->
            <div class="review-modal__content" id="modalContent"></div>
            ...
            <!-- Helpful -->
            <div class="review-modal__helpful">
                <span class="helpful-count"><i class="fas fa-thumbs-up"></i> <span id="modalHelpfulCount"></span> found this helpful</span>
                <form method="POST" action="" class="helpful-form" id="modalHelpfulForm">
                    <input type="hidden" name="csrf_token" value="<?php echo esc(appCsrfToken()); ?>">
                    <input type="hidden" name="review_id" value="" id="modalReviewId">
                    <button type="submit" name="action" value="toggle_like" class="helpful-btn" id="modalHelpfulBtn">
                        <i class="fas fa-thumbs-up" aria-hidden="true"></i> Mark Helpful
                    </button>
                </form>
            </div>
        </div>
...
(function() {
    ...
    const mAvatarPlaceholder = document.getElementById('modalAvatarPlaceholder');
    ...
    const mHelpfulBtn = document.getElementById('modalHelpfulBtn');
    ...
    function openModal(card) {
        ...
        const usernameEl = card.querySelector('.user-meta strong');
        const dateEl = card.querySelector('.user-meta > span');
        ...
        const holdingAmount = card.querySelector('.meta-item:nth-child(1) span')?.textContent || '';
        const holdingDays = card.querySelector('.meta-item:nth-child(2) span')?.textContent || '';
        const walletBadge = card.querySelector('.wallet-badge');
        const helpfulCount = card.querySelector('.helpful-count');
        const helpfulBtn = card.querySelector('.helpful-btn');
        const reviewIdInput = card.querySelector('input[name="review_id"]');

        // Avatar
        if (avatarImg && avatarImg.src) {
            mAvatar.src = avatarImg.src;
            mAvatar.alt = avatarImg.alt || '';
            mAvatar.hidden = false;
            mAvatarPlaceholder.hidden = true;
        } else if (avatarPlaceholder) {
            mAvatar.hidden = true;
            mAvatarPlaceholder.textContent = avatarPlaceholder.textContent;
            mAvatarPlaceholder.hidden = false;
        }

        // Username, level badge & date
        if (usernameEl) {
            const clone = usernameEl.cloneNode(true);
            const badge = clone.querySelector('.user-level-badge');
            if (badge) badge.remove();
            mUsername.textContent = clone.textContent.trim();
        }
        // Level badge
        const levelBadgeEl = card.querySelector('.user-level-badge');
        if (levelBadgeEl) {
            mLevelBadge.innerHTML = levelBadgeEl.outerHTML;
            mLevelBadge.hidden = false;
        } else {
            mLevelBadge.hidden = true;
        }
        mDate.textContent = dateEl ? dateEl.textContent.trim() : '';

        // Rating
        if (ratingEl) {
            mRating.innerHTML = ratingEl.innerHTML;
        } else {
            mRating.innerHTML = '';
        }
        ...
        // Project
        if (projectLink) {
            mProjectLink.href = projectLink.href;
        }
        mProjectName.textContent = projectNameEl ? projectNameEl.textContent.trim() : '';

        // Project statuses
        const projectStatusesEl = card.querySelector('.project-statuses');
        if (projectStatusesEl) {
            mProjectStatuses.innerHTML = projectStatusesEl.innerHTML;
            mProjectStatuses.hidden = false;
        } else {
            mProjectStatuses.hidden = true;
        }

        // Title
        mTitle.textContent = titleEl ? titleEl.textContent.trim() : '';
        mTitle.hidden = !titleEl;

        // Full content (get from data attribute or original)
        const fullContent = card.getAttribute('data-full-content') || (contentEl ? contentEl.innerHTML : '');
        // Convert newlines to <br> for display
        mContent.innerHTML = fullContent.replace(/\n/g, '<br>');

        // Meta
        mHoldingAmount.textContent = holdingAmount;
        mHoldingDays.textContent = holdingDays;
        if (walletBadge) {
            mWalletBadge.textContent = walletBadge.textContent.trim();
            mWalletBadge.className = 'wallet-badge ' + walletBadge.className.split(' ').filter(c => c.startsWith('wallet-badge') || c === 'non-custodial' || c === 'custodial' || c === 'unknown').join(' ');
        }

        // Helpful
        if (helpfulCount) {
            const countSpan = helpfulCount.querySelector('span') || helpfulCount;
            mHelpfulCount.textContent = countSpan.textContent.trim().replace(/[^0-9]/g, '');
        }
        if (reviewIdInput) {
            mReviewId.value = reviewIdInput.value;
        }
        if (helpfulBtn) {
            mHelpfulBtn.textContent = helpfulBtn.textContent.trim();
            mHelpfulBtn.className = helpfulBtn.className;
            if (helpfulBtn.hasAttribute('aria-pressed')) {
                mHelpfulBtn.setAttribute('aria-pressed', 'true');
            } else {
                mHelpfulBtn.removeAttribute('aria-pressed');
            }
        }
    ...
    // Bind "Read Full Review" buttons
    document.querySelectorAll('.read-more-btn[data-review-id]').forEach(function(btn) {
